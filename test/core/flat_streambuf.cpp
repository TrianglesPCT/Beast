...
public:
    ...
    testCtor()
    {
            ...
            Equal, Assign, Move, Swap, Select>;
        {
            ...
            basic_flat_streambuf<allocator> b1{1, 10};
            BEAST_EXPECT(b1.size() == 0);
            BEAST_EXPECT(b1.capacity() == 1);
        ...
        {
            basic_flat_streambuf<allocator> b1{allocator{}, 1, 10};
        }
    }
    ...
    testSpecialMembers()
    {
        ...
        {
            flat_streambuf fb{1, 10};
            BEAST_EXPECT(fb.max_size() == 10);
        }
        {
            flat_streambuf fb{1, 1024};
        ...
        for(std::size_t i = 1; i < s.size() - 1; ++i)
        {
            ...
            alloc_type alloc;
            {
                streambuf_type fba{alloc, 1, 1};
                BEAST_EXPECT(fba.max_size() == 1);
            }
            {
                streambuf_type fba{alloc, 1024, 1024};
    ...
    testStream()
    {
        ...
        flat_streambuf fb{1, 100};
        BEAST_EXPECT(fb.size() == 0);
        BEAST_EXPECT(fb.capacity() == 1);
